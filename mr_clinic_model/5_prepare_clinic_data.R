...
# iter all
if (F) {
    choose = sapply(1:2047, function(x) as.logical(intToBits(x)[1:11]))
    models = apply(choose, 2, function(ch) {
        subfeatures = features[ch]
        subtrain = train[,c(subfeatures, "time", "event")]
        cox = coxph(Surv(time, event) ~ . , data = subtrain)
        co = as.data.frame(summary(cox)$coefficients)
        if (nrow(co) == 0) {
            NULL
        } else if (all(co[,"Pr(>|z|)"] < 0.05)) {
            cox
        } else {
            NULL
        }
    })
    Ls = Filter(Negate(is.null), models)
}
