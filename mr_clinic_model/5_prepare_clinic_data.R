...
# for (w in c(0.9)) {
    L = lapply(1:4, function(k) {
        train = data[data$set == 0 & !data$name %in% valname[,k],]
        val = data[data$set == 0 & data$name %in% valname[,k],]
    ...
    citrs = mean(sapply(1:4, function(k) {
        tr = data[data$set == 0 & !data$name %in% valname[,k],]
        combine_pred(tr, L)
    }))
    civls = mean(sapply(1:4, function(k) {
        vl = data[data$set == 0 & data$name %in% valname[,k],]
        combine_pred(vl, L)
    }))
    result[w * n,] = c(w, citrs, civls)
    Ls[[length(Ls) + 1]] = L
}
saveRDS(Ls, "/home/tongxueqing/zhao/ImageProcessing/mr_clinic_model/_data/Ls_clinic.rds")
# combine_pred(test, L)
# combine_pred(newdata[newdata$set == 0,], L)
write.csv(result, "/home/tongxueqing/zhao/ImageProcessing/mr_clinic_model/_outs/weight_choose_clinic.out")