# -*- coding: utf-8 -*-
"""
Created on Wed Oct  9 16:06:28 2019
...
checkpoint_path = os.path.join(output_path, "checkpoint_path_focusLoss")
...
with tf.device('/cpu:0'):
    ...
    num_batchs = int(len(tra_path) / batch_size)
...
def main():
    ...
    with tf.device('/gpu:2'):
        ...
        weight1 = tf.multiply(tf.pow(tf.subtract(1., final_tensor), gamma),bias_class)
        weight2 = tf.multiply(weight1, new_labels)
    ...
    #        saver.restore(sess, "/home/competition/ZlZ-ShiPin/PythonApplication1/experiment_test/checkpoints/model_epoch10.ckpt") # 导入训练好的参数
            ...
            for epoch in range(num_epochs):
                ...
                train_loss = 0.0
                kk = 0
                np.random.shuffle(tra_path)
                for i in range(num_batchs-1):
                    img_paths = tra_path[i*batch_size:(i+1)*batch_size]

                    label_batch1 = [int(temp.split('_')[0]) for temp in img_paths]
                #    加主路径
                ...
                if (epoch+1) % 2 == 0:
                    ...
                    print(line)
                    with open(os.path.join(output_path,'logs_focusloss.txt'), 'a') as f:
                        f.write(line)
