...
#mRMR 考虑ICCs>=0.75
library('survcomp')
Fmrmr<-mrmr.cindex(x=tra.data,surv.time=msag1$time,surv.event=msag1$event,method="norther")
name1<-name1[rev(order(Fmrmr))]#rev provides a reversed version of its argument.order返回的是索引，sort返回的是排序好的值
name1 = name1[1:15]
...
myvars=name2 %in% 'log.sigma.5.0.mm.3D_glrlm_LongRunHighGrayLevelEmphasis'
name2=name2[!myvars]
