...
class Train(object):
    ...
    def __load_opt(self):
        return torch.optim.SGD(self.net.parameters(), lr = self.lr, weight_decay = self.weight_decay, momentum = 0.9, nesterov = True)
    ...
    def __evalu(self, loaders):
        self.net.eval()
        e = Evaluation()
        e.printplot(self.net, loaders, self.K)
        e.cnter("", self.K)
